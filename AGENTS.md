...
The `Canoe.xcodeproj` is generated by `Tools/generate_xcodeproj.py` (run via `just generate`). Do not edit the project file by hand - add or remove source files then regenerate. The generated project is committed to git. It carries no signing settings (no team, identity, or entitlements reference) - signing comes solely from `Justfile` env / `.env` at build time; regenerate if Xcode injects any.
